...
- Install dependencies with the system Bun, which matches the CI pin: `bun install --frozen-lockfile --ignore-scripts` (the canonical checkout already installed the Git hooks). Do not use `bun x bun@<version>`: Bun before 1.3.14 ignores the shared global store and copies about 0.5 GB into every worktree.
