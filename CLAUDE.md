...
Every inline keyboard callback that acts on a user-owned record **must** verify the caller is the intended recipient before doing anything. Do this as the first check, before any DB reads.

```ts
// BAD — anyone who knows the callback data can act on someone else's record
async function handleSecretaryAccept(id: number, deps: Deps) {
  const record = deps.repo.findById(id);
  if (!record) return;
  deps.repo.updateStatus(id, 'active'); // authorization bypass
}

// GOOD
async function handleSecretaryAccept(id: number, callerId: number, deps: Deps) {
  const record = deps.repo.findById(id);
  if (!record || record.secretary_id !== callerId) return;
  if (record.status !== 'pending') return;
  deps.repo.updateStatus(id, 'active');
}
```

Rule: callbacks that accept/decline/approve anything must receive `callerId` and check it against the record's expected recipient field (`secretary_id`, `target_id`, `invitee_id`, etc.).

### Status guard before state transitions
Before changing a record's status, check it is in the expected source state. Stale buttons in Telegram DMs are common — users press old buttons.

```ts
// BAD — re-activates already-declined records silently
if (!record) return;
...
When a security check function is optional or injected, the default must be **fail-closed** (deny), never fail-open (allow).

```ts
// BAD — if checker is absent, anyone passes
const allowed = ctx.checkMembership ? await ctx.checkMembership(chatId, userId) : true;

// GOOD
const allowed = ctx.checkMembership ? await ctx.checkMembership(chatId, userId) : false;
```

### Notification symmetry
If an action (accept/decline, approve/reject) sends a notification to party A, it must also send a notification to party B. Asymmetric notifications are always a bug.

```ts
// BAD — decline silently skips notifying the proposer
async function handleDecline(id: number) {
  repo.updateStatus(id, 'declined');
  await sendMessage(record.target_id, 'Вы отклонили предложение');
  // proposer never hears back
}
...
async function handleDecline(id: number) {
  repo.updateStatus(id, 'declined');
  await sendMessage(record.target_id, 'Вы отклонили предложение');
  await sendMessage(record.proposer_id, 'Ваше предложение отклонено');
}
```

### AI tool schema types must match handler types
The type declared in `input_schema` for an AI tool must match the TypeScript type used in the handler. Mismatch causes silent runtime bugs because casts hide them.

```ts
// BAD — schema says string, handler treats as number
// tools.ts
event_id: { type: 'string' }
// callback.handler.ts
const payload = JSON.parse(p) as { event_id?: number }; // lie

// GOOD — pick one and be consistent
// tools.ts
event_id: { type: 'number' }
// handler
const payload = JSON.parse(p) as { event_id?: number }; // matches
```

### Interface signatures must stay in sync
When you define an interface with a function type, every implementation and every call site must match the signature exactly. Removed parameters are bugs, not cleanup.

```ts
// BAD — interface removed 'text' param but implementation still has it
interface Sender { send(userId: number, username?: string): Promise<boolean> }
class RealSender { send(userId: number, text: string, username?: string) } // mismatch
```

### SQLite: wrap SELECT+UPDATE in a transaction when returning affected rows
`expirePending`-style patterns that SELECT to get IDs and then UPDATE must be wrapped in a transaction. Otherwise notifications fire for records that changed status between the two queries.

```ts
// BAD
expirePending() {
  const rows = db.prepare('SELECT ... WHERE status = ? AND expires_at < ...').all('pending');
  db.prepare('UPDATE ... SET status = ? WHERE ...').run('expired'); // TOCTOU
  return rows;
}
...
expirePending() {
  return db.transaction(() => {
    const rows = db.prepare('SELECT ...').all('pending');
    if (rows.length) db.prepare('UPDATE ...').run('expired');
    return rows;
  })();
}
```
...
Never construct a new repository instance if `DatabaseService` already exposes one (`db.secretaries`, `db.invitations`, etc.). One instance per repository per process.
