...
# 2. Patch setup.py — change release_kind() to return 'Debug'
#    (line 69: return 'Debug' if sys.platform != 'win32' else 'Release')

# 3. Patch cmake/macOS.cmake — add DEBUG to compile_definitions
#    (alongside NDEBUG, not replacing it)

# 4. Apply the fix to wrtc/src/interfaces/native_network_interface.cpp

# 5. Build
cd /tmp/ntgcalls-build
python3.12 -m venv /tmp/build-venv
/tmp/build-venv/bin/pip install setuptools wheel
/tmp/build-venv/bin/python setup.py build_ext --inplace

# 6. Install into project venv
cp ntgcalls.cpython-312-darwin.so /path/to/project/venv/lib/python3.12/site-packages/
```

Build auto-downloads all dependencies (WebRTC SDK, Chromium clang, Boost, FFmpeg, OpenH264).
Takes ~5 minutes on Apple Silicon.
