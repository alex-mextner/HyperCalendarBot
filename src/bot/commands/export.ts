...
export async function handleExport(ctx: BotCommandContext, eventService: EventService): Promise<void> {
  ...
  const file = new File([buffer], 'calendar.ics', { type: 'text/calendar' });
  await ctx.sendDocument(file, {
    caption: lang === 'ru' ? `📤 Экспортировано ${events.length} событий.` : `📤 Exported ${events.length} events.`,
  });
}
