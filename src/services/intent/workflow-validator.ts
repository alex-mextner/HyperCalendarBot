...
function extractAsFields(workflow: { [key: string]: unknown }): string[] {
  const found: string[] = [];
...
function extractAskUserNames(workflow: { [key: string]: unknown }): Set<string> {
 ...
 */
function extractStepOutputNames(workflow: { [key: string]: unknown }): Set<string> {
  const names = new Set<string>();
  const steps = workflow.steps;
  if (!Array.isArray(steps)) return names;
  for (const step of steps) {
