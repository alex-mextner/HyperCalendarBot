...
export function providerCircuitKey(provider: ProviderId, baseUrl: string, apiKey: string): string {
  // An API key is a random high-entropy token, not a user-chosen password; this digest is only a
  // stable account fingerprint used as a map key, and the key itself is never stored.
  // codeql[js/insufficient-password-hash] brute-forcing a random API key from it is infeasible.
