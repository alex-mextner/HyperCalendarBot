...
function updateCalls(ctx: AgentContext, updates: Record<string, unknown>): ToolResult {
  ...
  if (updates.enabled !== undefined)
    ctx.voice?.callSettingsRepo.setEnabled(ctx.user.telegram_id, updates.enabled as boolean);
  if (updates.language !== undefined)
    ctx.voice?.callSettingsRepo.setLanguage(ctx.user.telegram_id, updates.language as string);
