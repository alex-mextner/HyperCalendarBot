...
export function getToolDefinitions(inputMode?: string, caps?: UserCapabilities): ToolDefinition[] {
  const base =
    inputMode === 'live_call'
      ? toolDefinitions.filter((t) => !CALL_EXCLUDED_TOOLS.has(t.name))
      : toolDefinitions.filter((t) => !CALL_ONLY_TOOLS.has(t.name));

  if (caps?.assistantEnabled === true && caps?.agentConnected === true) {
    return [...base, ...assistantToolDefinitions];
  }
  return base;
}
