...
interface GetDhConfigPayload {
  _: 'messages.getDhConfig';
  version: number;
  randomLength: number;
}

/** Union of all MTProto method payloads used by VoiceCallOrchestrator. */
...
export function computeEmojiFingerprint(key: Buffer, gA: Buffer): string[] {
  ...
  const EMOJI_COUNT = BigInt(EMOJI_LIST.length);
  const emojis: string[] = [];
  for (let i = 0; i < 4; i++) {
    const offset = i * 8;
    let value = 0n;
...
// ---------------------------------------------------------------------------
// Full 333 emoji list for visual key verification (from tdesktop source).
// https://core.telegram.org/api/end-to-end/voice-calls#emoji-visualization
// ---------------------------------------------------------------------------
