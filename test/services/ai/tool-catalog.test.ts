...
describe('capability-scoped batched tool discovery', () => {
  ...
  test('preserves supplement exclusions and newly added allowed tools', () => {
    const catalog = createToolCatalog(getToolDefinitions('text', undefined, true));
  ...
  test('round-trips every canonical schema across capability and mode variants', () => {
    for (const mode of ['text', 'live_call'] as const)
      for (const enabled of [false, true])
        for (const supplement of [false, true]) {
          const tools = getToolDefinitions(mode, { assistantEnabled: enabled }, supplement);
  ...
  test('interleaved catalogs never share capabilities or mutable nested input', () => {
    const tools = getToolDefinitions('text', { assistantEnabled: true });
    const enabled = createToolCatalog(tools);
    const disabled = createToolCatalog(getToolDefinitions('text'));
    const before = enabled.describe({ tools: ['get_events'] });
    const first = tools[0]!;
    if (first.type !== 'function') throw new Error('fixture');
    first.function.parameters = { injected: true };
    const returned = enabled.describe({ tools: ['get_events'] });
    ...
    expect(enabled.describe({ tools: ['get_events'] })).toEqual(before);
    expect(enabled.index()).toContain('bash_execute');
    expect(disabled.index()).not.toContain('bash_execute');
    const denied = disabled.describe({ tools: ['bash_execute'] });
