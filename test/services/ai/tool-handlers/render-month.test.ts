...
describe('handleRenderMonthImage', () => {
  test('returns success with rendering message', () => {
    const ctx = makeCtx();
    const result = handleRenderMonthImage(ctx, { month: '2026-04' });
    expect(result.success).toBe(true);
    expect(result.output).toBeTruthy();
  });

  test('output contains month', () => {
    const ctx = makeCtx();
    const result = handleRenderMonthImage(ctx, { month: '2026-04' });
  ...
  test('fails when group scope but no groupChatId', () => {
    ...
    const result = handleRenderMonthImage(ctx, { month: '2026-04', scope: 'group' });
    expect(result.success).toBe(false);
  });
  ...
  test('calls getEventsInRangeForGroup when scope is group', async () => {
    ...
    handleRenderMonthImage(ctx, { month: '2026-04', scope: 'group' });
    await flushPromises();
    expect(getEventsInRangeForGroup).toHaveBeenCalled();
  });

  test('accepts YYYY-MM-DD format and parses correctly', () => {
    const ctx = makeCtx();
    const result = handleRenderMonthImage(ctx, { month: '2026-04-01' });
    expect(result.success).toBe(true);
  });
