...
describe('NotificationScheduler', () => {
  ...
  test('clock-change appended to morning agenda when morning agenda user has no events on DST day', async () => {
    // User has morning agenda enabled but no events today — morning agenda still sent with clock-change appended
    db.run("INSERT INTO users (telegram_id, timezone, language) VALUES (42, 'Europe/Berlin', 'en')");
    db.run("INSERT INTO notification_preferences (user_id, morning_agenda_time) VALUES (42, '08:00')");
    // No events inserted
    let capturedType = '';
