...
describe('ReminderMaterializer', () => {
  ...
  test('materializes reminders using default intervals', () => {
    ...
    materializer.materialize({ id: 1, start_at: '2099-03-15T10:00:00Z', reminder_overrides: null }, 42);
  ...
  test('uses event-level overrides when present', () => {
    db.run("INSERT INTO events (id, user_id, title, start_at) VALUES (1, 42, 'Meeting', '2099-03-15T10:00:00Z')");
    materializer.materialize({ id: 1, start_at: '2099-03-15T10:00:00Z', reminder_overrides: '[5, 60]' }, 42);
    const rows = reminderRepo.getForEvent(1);
  ...
  test('skips reminders in the past', () => {
    ...
    materializer.materialize({ id: 1, start_at: '2020-01-01T10:00:00Z', reminder_overrides: null }, 42);
    const rows = reminderRepo.getForEvent(1);
  ...
  test('re-materializing replaces old reminders', () => {
    db.run("INSERT INTO events (id, user_id, title, start_at) VALUES (1, 42, 'Meeting', '2099-03-15T10:00:00Z')");
    materializer.materialize({ id: 1, start_at: '2099-03-15T10:00:00Z', reminder_overrides: null }, 42);
    materializer.materialize({ id: 1, start_at: '2099-03-15T10:00:00Z', reminder_overrides: '[5]' }, 42);
    const rows = reminderRepo.getForEvent(1);
    expect(rows.length).toBe(1);
