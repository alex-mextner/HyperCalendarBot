...
// Real browser lifecycle cannot share timer/global mocks with unrelated unit tests.
// Run the exact six integration cases in a separate process, without a skip fallback.
if (process.env.HCB_REAL_POOL_CHILD !== '1') {
  test('real Chromium pool integration passes in an isolated process', () => {
    const run = spawnSync(process.execPath, ['--no-env-file', 'test', fileURLToPath(import.meta.url)], {
      env: { ...process.env, HCB_REAL_POOL_CHILD: '1' },
      encoding: 'utf8',
      timeout: 25000,
    });
    ...
    expect(report).toMatch(/\b6 pass\b/);
    expect(report).toMatch(/\b0 fail\b/);
    expect(report).not.toMatch(/\b[1-9]\d* skip\b/);
  }, 30000);
} else {
